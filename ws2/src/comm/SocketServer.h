...
class SocketServer {
public:
    SocketServer(const std::string& ip, int port, Poco::JSON::Array::Ptr& ebikes)
        : ip_(ip), port_(port), ebikes_(ebikes), running_(false) {}
...
private:
    void run() {
        try {
            ...
            std::cout << "[SOCKETS] " << ip_ << " Socket server started on:" << port_ << "." << std::endl;
